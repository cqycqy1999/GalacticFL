...
class GeneralClient:
    def __init__(self, client_id, model, data_path, output_dir_path):
        ...
        self.local_output_dir_path = os.path.join(self.output_dir_path, "trainer_saved", f"local_output_{self.client_id}")

    def prepare_local_dataset(self, generate_and_tokenize_prompt, local_val_setsize):
        if local_val_setsize > 0:
            local_train_val = self.local_data["train"].train_test_split(
                test_size=local_val_setsize, shuffle=True, seed=42
            )
        ...
        self.local_val_setsize = local_val_setsize
    ...
    def build_local_trainer(self,
        ...
        self.train_args = transformers.TrainingArguments(
            ...
            logging_steps=1,
            optim="adamw_torch",
            evaluation_strategy="steps" if self.local_val_setsize > 0 else "no",
            save_strategy="steps",
            eval_steps=200 if self.local_val_setsize > 0 else None,
            save_steps=5000000,
            output_dir=self.local_output_dir_path,
            save_total_limit=1,
            load_best_model_at_end=True if self.local_val_setsize > 0 else False,
    ...
    def initiate_local_training(self):
        self.model.config.use_cache = False # KVcache，在训练时候需要关闭
    ...
    def terminate_local_training(self, epoch, local_dataset_len_dict, previously_selected_clients_set):
        ...
        # if "Dolly" in self.local_data_path:
        #     dataset_name = "Dolly"
        # elif "CodeAlpaca" in self.local_data_path:
